...
export const getBreadcrumbItems = (currentPath) => {
  ...
  const items = [];
  const normalizedPath = currentPath.startsWith('/views') ? currentPath : `/views${currentPath}`;
  const isAdminPath = normalizedPath.includes('/views/admin');
  const navigationItems = isAdminPath ? adminItems : educatorItems;
  const basePath = <i class="bi bi-house"></i>;
  const baseLink = isAdminPath ? '/views/admin/dashboard' : '/views/educator/dashboard';

  const findMatchingItem = (navItems, path, breadcrumbPath = []) => {
    for (const item of navItems) {
      const itemPath = item.path?.startsWith('/views') ? item.path : `/views${item.path}`;
      
      // Check if this is a settings parent item (disabled)
      const isDisabled = (item.id === 'settings' || item.id === 'datamanagement' || item.id === 'postanalytics') && item.subItems;
      ...
      if (item.subItems) {
        const result = findMatchingItem(
          item.subItems, 
          path, 
          [...breadcrumbPath, { ...item, disabled: isDisabled }]
        );
  ...
  if (matchingPath) {
    matchingPath.forEach((item, index) => {
      if (index < matchingPath.length - 1) {
        ...
        items.push({
          text: item.label,
          link: item.disabled ? null : (item.path?.startsWith('/views') ? item.path : `/views${item.path}`),
