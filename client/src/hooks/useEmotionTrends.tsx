...
export function useEmotionTrends() {
  const [trendData, setTrendData] = useState<DailyEmotion[]>([]);
  ...
  useEffect(() => {
    async function fetchTrends() {
      ...
      try {
        ...
        const dailyMap: Record<string, DailyEmotion> = {};
        ...
        const trendArr = Object.values(dailyMap).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
